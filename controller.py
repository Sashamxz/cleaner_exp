...
class Controller:
    def __init__(self, model, view, expanc_file):
        ...
        # self.view.filemenu.add_command(label="Открыть",
        #                                command= self.model.open_file)
        self.view.filemenu.add_command(label="Узнать расширение",
                                       command=self.open_window_file)
        self.view.filemenu.add_command(label="win/lin/mack",
                                       command=self.model.winlin)
        self.view.filemenu.add_command(label="Выход",
                                       command=self.model.exit_prog)
        self.view.helpmenu.add_command(label="Помощь",
                                       command=self.open_help)
        self.view.helpmenu.add_command(label="О программе",
                                       command=self.open_about_prog)
