#!/bin/usr/env python3
# verion 0.3.2
# Програма для перемещения файлов по расширению из
# исходной папки(src) в папку назначения(dst)
...
ent_dst_title = tk.Label(root,
                         text='Введите абсолютный путь к каталогу назначения:')
ent_src_title = tk.Label(root,
                         text='Введите абсолютный путь к исходному каталогу:')
ent_expansion_title = tk.Label(root,
                               text='Введите расширения файла, например-(.txt)\
                               или " * " что бы выбрать все файлы: ')
...
def cleaner(src_adr, dst_adr, expancion_adr):
    ...
    for _file in path_file:
        # выбираем  фалы по расширению,!=dst
        if _file.endswith(expancion_adr) and os.path.abspath(_file) != dst_adr:
...
def show_expancion():
    ...
    var = tk.StringVar
    window_text = tk.Label(window,
                           text='Выберите файл чтобы узнать его расширение')
...
def winlin():
    ...
    if platformf == "linux" or platformf == "linux2":
        mb.showinfo("OS", "Linux :)")

    elif platformf == "darwin":
        mb.showinfo("OS", "macOS")

    elif platformf == "win32":
        mb.showinfo("OS", "Windows...")
...
filemenu.add_command(label="Открыть",
                     command=open)
filemenu.add_command(label="Узнать расширение",
                     command=show_expancion)
filemenu.add_command(label="win/lin/mack",
                     command=winlin)
...
def about_prog():
    ...
    window.geometry('600x400')
    window.title('About')
    about = 'Cleaner expansion-програма для сортировки \
            файлов по рассширению \n  \
            Version - %s' % (__version)
    text_help = tk.Label(window, text=about)
    text_help.pack(side=tk.LEFT, expand=True, padx=10, pady=10)
    window.mainloop()
...
helpmenu.add_command(label="Помощь",
                     command=help_user)
but = tk.Button(root, text="Старт",
                bg="lightgreen", command=main)
